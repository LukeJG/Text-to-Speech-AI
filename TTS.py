...
class VCTKDataset(Dataset):
    def __init__(self, root_dir=None, max_len=MAX_TEXT_LENGTH):
        ...
        if not os.path.exists(wav_dir):
            wav_dir = os.path.join(self.root_dir, "wav48") 
            if not os.path.exists(wav_dir):  
    ...
    def __getitem__(self, idx):
        ...
        if len(audio) > MAX_WAV_LENGTH * SAMPLE_RATE:      
            audio = audio[:MAX_WAV_LENGTH * SAMPLE_RATE]

        mel_spec = self._get_mel_spectrogram(audio)    
    ...
    def forward(self, encoder_output, mel_targets=None, teacher_forcing_ratio=0.5):
        ...
        if mel_targets is None:
            ...
            for t in range(max_len):
                decoder_output1, (h1, c1) = self.decoder_rnn1(decoder_input, (h1, c1))  
                ...
                decoder_output2, (h2, c2) = self.decoder_rnn2(attn_output, (h2, c2))  

                output = self.proj(decoder_output2)
                outputs[:, t:t+1] = output

                decoder_input = decoder_output2  
...
class TTSModel(nn.Module):
    ...
    def forward(self, input_ids, speaker_ids, attention_mask=None, mel_targets=None, teacher_forcing_ratio=0.5):
        encoder_output = self.text_encoder(input_ids, attention_mask)  

        speaker_embed = self.speaker_encoder(speaker_ids)  
...
def griffin_lim_vocoder(mel_spectrogram, n_iter=32):
    mel_basis = librosa.filters.mel(sr=SAMPLE_RATE, n_fft=N_FFT, n_mels=MEL_CHANNELS) 
    mel_to_linear = np.linalg.pinv(mel_basis)

    mel_spec = mel_spectrogram.numpy()
    mel_spec = np.exp(mel_spec)

    linear_spec = np.dot(mel_to_linear, mel_spec)  
